...
class LSTMLayer(MergeLayer):
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        mask = None
        # hid_init = theano.clone(self.hid, share_inputs=False)
        # cell_init = theano.clone(self.cell, share_inputs=False)
        hid_init = self.hid
        cell_init = self.cell
        ...
        cell_init = T.dot(ones, cell_init)

        # if not isinstance(self.hid_init, Layer):
        # Dot against a 1s vector to repeat to shape (num_batch, num_units)
        # hid_init = T.dot(ones, self.hid_init)
        hid_init = T.dot(ones, hid_init)
